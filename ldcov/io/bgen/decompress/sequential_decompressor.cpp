...
SequentialDecompressor::SequentialDecompressor(const SequentialConfig& config)
    ...
    if (config.buffer_manager) {
        ...
        bm_config.max_buffer_size = std::max(config.readahead_buffer_size * 2,
...
DecompressedData SequentialDecompressor::decompress(const CompressedVariant& variant) {
    try {
        ...
        if (variant.data != nullptr) {
            ...
            auto result = decompress_data(variant.data,
                                          variant.compressed_size,
                                          variant.uncompressed_size,
                                          variant.compression_type,
                                          variant.offset);
        ...
        if (!compressed_buffer.valid()) {
            ...
            return DecompressedData(variant.offset, 
                                    DecompressedData::MEMORY_ERROR,
                                    "Failed to read compressed data");
        }
        
        // Decompress the data
        auto result = decompress_data(compressed_buffer.data(),
                                      variant.compressed_size,
                                      variant.uncompressed_size,
                                      variant.compression_type,
                                      variant.offset);
        ...
        update_statistics(variant, false);
        return DecompressedData(variant.offset,
                                DecompressedData::COMPRESSION_ERROR,
...
bool SequentialDecompressor::has_prefetched_data(uint64_t offset, size_t size) const {
    ...
    return current_info.valid && 
           current_info.offset == offset && 
...
BufferHandle SequentialDecompressor::read_compressed_data(const CompressedVariant& variant) {
    ...
    // Read data
    size_t bytes_read = file_reader_->read_at(variant.offset, current_buffer.data(), 
                                           variant.compressed_size);
...
DecompressedData SequentialDecompressor::decompress_data(
    ...
    if (compression_type == CompressionType::None) {
        if (compressed_size != expected_size) {
            ...
            oss << "Uncompressed size mismatch: expected " << expected_size
                << ", got " << compressed_size;
    ...
    switch (compression_type) {
        case CompressionType::Zlib:
            result = decompress_zlib(compressed, compressed_size,
                                     decomp_buffer.data(), expected_size);
            break;
            
        case CompressionType::Zstd:
            result = decompress_zstd(compressed, compressed_size,
                                     decomp_buffer.data(), expected_size);
            break;
    ...
    if (!result.success) {
        return DecompressedData(offset, DecompressedData::COMPRESSION_ERROR,
    ...
    if (config_.validate_size && result.bytes_processed != expected_size) {
        std::ostringstream oss;
        oss << "Decompressed size mismatch: expected " << expected_size
            << ", got " << result.bytes_processed;
        return DecompressedData(offset, DecompressedData::SIZE_MISMATCH, oss.str());
    }
...
DecompressedData SequentialDecompressor::decompress_direct(const CompressedVariant& variant) {
    ...
    size_t bytes_read = file_reader_->read_at(variant.offset, direct_read_buffer_.get(), 
                                             variant.compressed_size);
    if (bytes_read != variant.compressed_size) {
        return DecompressedData(variant.offset, 
                                DecompressedData::MEMORY_ERROR,
    ...
    switch (variant.compression_type) {
        case CompressionType::None:
            ...
            if (variant.compressed_size != variant.uncompressed_size) {
                return DecompressedData(variant.offset, 
                                        DecompressedData::SIZE_MISMATCH,
        ...
        case CompressionType::Zstd: {
            ...
            size_t result = ZSTD_decompressDCtx(dctx,
                                                output_buffer.data(), variant.uncompressed_size,
        ...
        default:
            return DecompressedData(variant.offset, 
                                    DecompressedData::UNSUPPORTED_COMPRESSION,
                                    "Unsupported compression type");
    }
...
DecompressedData SequentialDecompressor::decompress_uncompressed_direct(const CompressedVariant& variant) {
    // Validate size
    if (variant.compressed_size != variant.uncompressed_size) {
        return DecompressedData(variant.offset, 
                                DecompressedData::SIZE_MISMATCH,
    ...
    size_t bytes_read = file_reader_->read_at(variant.offset, output.get(), variant.compressed_size);
    if (bytes_read != variant.compressed_size) {
        return DecompressedData(variant.offset, 
                                DecompressedData::MEMORY_ERROR,
