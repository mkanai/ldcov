...
bool SequentialFastPath::decompress_variant(const uint8_t* compressed, size_t compressed_size,
    ...
    switch (type) {
        case CompressionType::None:
            ...
            if (compressed_size != uncompressed_size) {
                snprintf(last_error_, ERROR_MSG_SIZE,
                         "Uncompressed size mismatch: expected %zu, got %zu", uncompressed_size,
                         compressed_size);
                return false;
            }
        ...
        case CompressionType::Zlib: {
            ...
            if (ret != Z_OK || dest_len != uncompressed_size) {
                snprintf(last_error_, ERROR_MSG_SIZE, "Zlib decompression failed: %d", ret);
                return false;
            }
            return true;
        }
        ...
        case CompressionType::Zstd: {
            ...
            size_t ret = ZSTD_decompressDCtx(static_cast<ZSTD_DCtx*>(zstd_dctx_), output,
                                             uncompressed_size, compressed, compressed_size);

            if (ZSTD_isError(ret) || ret != uncompressed_size) {
                snprintf(last_error_, ERROR_MSG_SIZE, "Zstd decompression failed: %s",
        ...
        default:
            snprintf(last_error_, ERROR_MSG_SIZE, "Unknown compression type: %d",
                     static_cast<int>(type));
            return false;
    }
}

bool SequentialFastPath::decompress_direct(uint64_t file_offset, size_t compressed_size,
    ...
    if (bytes_read != compressed_size) {
        snprintf(last_error_, ERROR_MSG_SIZE, "Read failed: expected %zu bytes, got %zu",
    ...
    return decompress_variant(read_buffer_.get(), compressed_size, uncompressed_size,
                              compression_type, output);
}

size_t SequentialFastPath::decompress_batch_direct(const uint64_t* offsets,
                                                   const size_t* compressed_sizes,
                                                   const size_t* uncompressed_sizes,
                                                   const CompressionType* compression_types,
                                                   uint8_t** outputs, size_t n_variants) {
    size_t successful = 0;
    ...
    for (size_t i = 0; i < n_variants; ++i) {
        ...
        if (decompress_direct(offsets[i], compressed_sizes[i], uncompressed_sizes[i],
                              compression_types[i], outputs[i])) {
            successful++;
        } else {
