...
std::unique_ptr<ParallelDecompressor::DecompressionTask> 
ParallelDecompressor::TaskQueue::pop() {
...
ParallelDecompressor::ResultCollector::collect_results(size_t count) {
    std::unique_lock<std::mutex> lock(mutex_);
    ...
    // Check for timeout
    if (!success) {
        throw std::runtime_error("Timeout waiting for decompression results after 30 seconds. "
...
ParallelDecompressor::ParallelDecompressor(FileReader* file_reader, const ParallelConfig& config)
    ...
    for (size_t i = 0; i < num_threads; ++i) {
        ...
        // Allocate I/O buffer
        worker->io_buffer = std::unique_ptr<uint8_t[]>(new uint8_t[parallel_config_.io_buffer_size]);
        
        // Start worker thread
        worker->thread = std::thread(&ParallelDecompressor::worker_thread_function, this, worker.get());
    ...
    std::cerr << "ParallelDecompressor initialized with " << num_threads << " worker threads" << std::endl;
    #endif
}
...
std::vector<DecompressedData> 
ParallelDecompressor::decompress_batch(const std::vector<CompressedVariant>& variants) {
    if (variants.empty()) {
        return {};
    }
    ...
    for (size_t i = 0; i < variants.size(); ++i) {
        ...
        while (task_queue_.size() >= parallel_config_.max_queue_size) {
            ...
            if (elapsed > std::chrono::seconds(5)) {
                throw std::runtime_error("Timeout waiting for task queue space. Queue may be stuck.");
            }
        }
...
DecompressedData ParallelDecompressor::decompress_variant(const CompressedVariant& variant,
    ...
    try {
        ...
        if (!variant.data && variant.compressed_size > 0) {
            ...
            if (!file_reader_->is_open()) {
                return DecompressedData(variant.offset, 
                    DecompressedData::INVALID_INPUT,
            ...
            size_t bytes_read = file_reader_->read_at(variant.offset, 
                                                   state->io_buffer.get(), 
        ...
        auto buffer_handle = state->buffer_manager->get_decompression_buffer(variant.uncompressed_size);
