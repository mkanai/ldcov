...
public:
    ...
    BufferHandle(std::unique_ptr<uint8_t[]> buffer, size_t size, 
                 BufferManager* manager, std::function<void(std::unique_ptr<uint8_t[]>, size_t)> return_func);
...
private:
    ...
    struct PoolEntry {
        ...
        PoolEntry(std::unique_ptr<uint8_t[]> buf, size_t sz)
