...
void SimdUtils::convert_probabilities_to_dosages_8bit(
    const uint8_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
...
void SimdUtils::convert_probabilities_to_dosages_16bit(
...
void SimdUtils::convert_probabilities_scalar_8bit(
    const uint8_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
...
void SimdUtils::convert_probabilities_scalar_16bit(
    const uint16_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
...
#ifdef LDCOV_HAS_AVX2
void SimdUtils::convert_probabilities_avx2_8bit(
    const uint8_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
    ...
    for (; i + 8 <= n_samples; i += 8) {
        ...
        __m128i p01_bytes = _mm_setr_epi8(
            probs[i*3 + 1], probs[(i+1)*3 + 1], probs[(i+2)*3 + 1], probs[(i+3)*3 + 1],
            probs[(i+4)*3 + 1], probs[(i+5)*3 + 1], probs[(i+6)*3 + 1], probs[(i+7)*3 + 1],
            0, 0, 0, 0, 0, 0, 0, 0
        );
        
        // Extract p11 values (every 3rd byte starting at offset 2)
        __m128i p11_bytes = _mm_setr_epi8(
            probs[i*3 + 2], probs[(i+1)*3 + 2], probs[(i+2)*3 + 2], probs[(i+3)*3 + 2],
            probs[(i+4)*3 + 2], probs[(i+5)*3 + 2], probs[(i+6)*3 + 2], probs[(i+7)*3 + 2],
            0, 0, 0, 0, 0, 0, 0, 0
        );
...
void SimdUtils::convert_probabilities_sse2_8bit(
    const uint8_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
...
void SimdUtils::convert_probabilities_neon_8bit(
    const uint8_t* probs,
    float* dosages,
    size_t n_samples,
    uint8_t ploidy) {
