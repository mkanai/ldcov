...
private:
    ...
    struct DecompressionTask {
        ...
        DecompressionTask(size_t id, CompressedVariant v)
            : task_id(id), variant(std::move(v)) {}
    };
    ...
    DecompressedData decompress_variant(const CompressedVariant& variant,
                                       WorkerState* state);
    ...
    void prefetch_variants(const std::vector<CompressedVariant>& variants,
                          size_t start_idx);
