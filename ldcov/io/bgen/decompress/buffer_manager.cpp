...
BufferHandle::BufferHandle(std::unique_ptr<uint8_t[]> buffer, size_t size, BufferManager* manager,
...
BufferManager::BufferManager(const Config& config) : config_(config) {
...
BufferHandle BufferManager::get_buffer(size_t required_size) {
    ...
    // Try to get a buffer from the pool
    {
        ...
        auto it = std::find_if(pool_.begin(), pool_.end(), [required_size](const PoolEntry& entry) {
    ...
    if (!buffer) {
        ...
        if (config_.enable_statistics) {
            ...
            size_t current =
                current_memory_usage_.fetch_add(buffer_size, std::memory_order_relaxed) +
                buffer_size;
            size_t peak = stats_.peak_memory_allocated.load(std::memory_order_relaxed);
            while (current > peak &&
                   !stats_.peak_memory_allocated.compare_exchange_weak(peak, current)) {
