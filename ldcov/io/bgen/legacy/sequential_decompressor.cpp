...
inline bool SequentialDecompressor::decompress_zstd_inline(const uint8_t* compressed_data,
                                                          ...
                                                          size_t expected_size,
                                                          size_t* actual_size) {
    size_t result = ZSTD_decompress(output_buffer, expected_size,
...
DecompressionResult SequentialDecompressor::decompress_variant(
    uint64_t offset,
    ...
    switch (compression_type) {
        case 1:  // zlib
            decomp_success = decompress_zlib_inline(work_buffer + 4, payload_size,
                                                   decomp_buffer, uncompressed_size, &actual_size);
            break;
        case 2:  // zstd
...
DecompressionResult SequentialDecompressor::decompress_variant_allocated(
    uint64_t offset,
    uint32_t compressed_size,
    uint32_t expected_size,
    uint8_t compression_type) {
    ...
    DecompressionResult temp_result = decompress_variant(offset, compressed_size, expected_size, compression_type);
...
std::vector<DecompressionResult> SequentialDecompressor::decompress_sequential(
    ...
    bool use_readahead = enable_readahead && n_variants > 1 && 
                        is_sequential_pattern(offsets);
    ...
    for (size_t i = 0; i < n_variants; ++i) {
        ...
        if (use_readahead && i > 0 && read_ahead_->is_active && 
            read_ahead_->offset == offsets[i]) {
            ...
            if (bytes_read == static_cast<ssize_t>(compressed_sizes[i])) {
                ...
                if (compression_types[i] == 0) {
                    ...
                    // Read the uncompressed size from the first 4 bytes
                    uint32_t uncompressed_size = *reinterpret_cast<uint32_t*>(read_ahead_->buffer.get());
                    ...
                    switch (compression_types[i]) {
                        case 1:  // zlib
                            decomp_success = decompress_zlib_inline(read_ahead_->buffer.get() + 4, 
                                                                   payload_size,
                                                                   decomp_buffer, uncompressed_size, 
                                                                   &actual_size);
                            break;
                        case 2:  // zstd
                            decomp_success = decompress_zstd_inline(read_ahead_->buffer.get() + 4, 
                                                                   payload_size,
                                                                   decomp_buffer, uncompressed_size, 
                                                                   &actual_size);
                            break;
                ...
                result = decompress_variant(offsets[i], compressed_sizes[i], 
                                          expected_sizes[i], compression_types[i]);
            }
        } else {
