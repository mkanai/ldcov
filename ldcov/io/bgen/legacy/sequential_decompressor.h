...
class SequentialDecompressor {
   ...
   public:
    ...
    DecompressionResult decompress_variant(uint64_t offset, uint32_t compressed_size,
                                           uint32_t expected_size, uint8_t compression_type);
    ...
    DecompressionResult decompress_variant_allocated(uint64_t offset, uint32_t compressed_size,
    ...
    uint64_t total_variants_processed() const {
        return total_variants_processed_;
    }
    double read_amplification() const {
        return total_bytes_decompressed_ > 0
                   ? static_cast<double>(total_bytes_read_) / total_bytes_decompressed_
                   : 0.0;
    }
};
