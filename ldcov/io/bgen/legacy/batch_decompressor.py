            ...
            if HAS_SEQUENTIAL_DECOMPRESSOR or HAS_CYTHON_DECOMPRESSOR:
                ...
                try:
                    ...
                    if file_size_mb > 3000:
                        use_cpp = True
                        logger.debug(f"Using C++ decompressor for biobank-scale {file_size_mb:.1f}MB file")
                    # Small files: Python is faster
                        ...
                        logger.debug(f"Using {'C++' if use_cpp else 'Python'} for uncompressed {file_size_mb:.1f}MB file")
                    elif compression_type == 1:  # zlib
                        # C++ beneficial for medium files
                        use_cpp = 100 <= file_size_mb <= 500
                        logger.debug(f"Using {'C++' if use_cpp else 'Python'} for zlib {file_size_mb:.1f}MB file")
                    elif compression_type == 2:  # zstd
                        # Only beneficial for very large files
                        use_cpp = file_size_mb > 900
                        logger.debug(f"Using {'C++' if use_cpp else 'Python'} for zstd {file_size_mb:.1f}MB file")
                    else:
                        # Unknown compression, use Python
                        use_cpp = False
                        logger.debug(f"Using Python for unknown compression type {compression_type}")
                    ...
                    self._use_python = True
                    logger.debug("Using Python read-ahead (failed to determine file characteristics)")
            else:
                self._use_python = True
                logger.debug("Using Python read-ahead (no C++ decompressor available)")
                
    def process_variants(self, offsets: List[int], variant_metadata: List = None,
                        sample_indices: Optional[np.ndarray] = None,
                        progress_callback: Optional[Callable[[int], None]] = None
                        ) -> Tuple[np.ndarray, List]:
        """
...
def should_use_decompressor(file_size_mb: float, compression_type: int, 
                           access_pattern: str = 'sequential') -> Tuple[bool, str]:
    """
