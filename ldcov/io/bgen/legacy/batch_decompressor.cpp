...
void BatchDecompressor::worker_thread_func(int thread_id) {
    ...
    while (true) {
        ...
        if (task.compression_type == 0) {
            ...
            ssize_t bytes_read = ::pread(fd, owned_result.owned_data.get(), 
                                        task.compressed_size, task.offset);
            ...
            switch (task.compression_type) {
                case 1:  // zlib
                    decomp_success = decompress_zlib_inline(
                        work_buffer + 4, payload_size,
                        owned_result.owned_data.get(), uncompressed_size,
                        &actual_size);
                    break;
                case 2:  // zstd
