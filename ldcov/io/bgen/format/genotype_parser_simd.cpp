...
void compute_dosages_8bit_simd(const uint8_t* prob_data, float* output, size_t n_samples,
    ...
    if (g_has_avx2) {
        ...
        for (; i + 15 < n_samples; i += 16) {
            ...
            __m256i mask_even =
                _mm256_set_epi8(30, 28, 26, 24, 22, 20, 18, 16, 14, 12, 10, 8, 6, 4, 2, 0, 30, 28,
                                26, 24, 22, 20, 18, 16, 14, 12, 10, 8, 6, 4, 2, 0);
            __m256i mask_odd =
                _mm256_set_epi8(31, 29, 27, 25, 23, 21, 19, 17, 15, 13, 11, 9, 7, 5, 3, 1, 31, 29,
            ...
            __m256 prob_ab_f =
                _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(prob_ab_lo)));
            __m256 prob_bb_f =
                _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_castsi256_si128(prob_bb_lo)));
            ...
            prob_ab_f =
                _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(prob_ab_lo, 1)));
            prob_bb_f =
                _mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(_mm256_extracti128_si256(prob_bb_lo, 1)));
...
void compute_dosages_16bit_simd(const uint8_t* prob_data, float* output, size_t n_samples,
    ...
    if (g_has_avx2) {
        ...
        for (; i + 7 < n_samples; i += 8) {
            ...
            __m256i data_lo =
                _mm256_loadu_si256(reinterpret_cast<const __m256i*>(prob_data + i * 4));
            __m256i data_hi =
                _mm256_loadu_si256(reinterpret_cast<const __m256i*>(prob_data + i * 4 + 16));
            ...
            __m256i prob_aa = _mm256_set_epi32(
                read_le16(prob_data + (i + 7) * 4), read_le16(prob_data + (i + 6) * 4),
                read_le16(prob_data + (i + 5) * 4), read_le16(prob_data + (i + 4) * 4),
                read_le16(prob_data + (i + 3) * 4), read_le16(prob_data + (i + 2) * 4),
                read_le16(prob_data + (i + 1) * 4), read_le16(prob_data + (i + 0) * 4));

            // Gather P(AB) values (every other 16-bit value starting at offset 2)
            __m256i prob_ab = _mm256_set_epi32(
                read_le16(prob_data + (i + 7) * 4 + 2), read_le16(prob_data + (i + 6) * 4 + 2),
                read_le16(prob_data + (i + 5) * 4 + 2), read_le16(prob_data + (i + 4) * 4 + 2),
                read_le16(prob_data + (i + 3) * 4 + 2), read_le16(prob_data + (i + 2) * 4 + 2),
                read_le16(prob_data + (i + 1) * 4 + 2), read_le16(prob_data + (i + 0) * 4 + 2));
    ...
    if (g_has_neon) {
        ...
        for (; i + 3 < n_samples; i += 4) {
            ...
            uint16x4_t prob_aa = {
                read_le16(prob_data + (i + 0) * 4), read_le16(prob_data + (i + 1) * 4),
                read_le16(prob_data + (i + 2) * 4), read_le16(prob_data + (i + 3) * 4)};

            uint16x4_t prob_ab = {
                read_le16(prob_data + (i + 0) * 4 + 2), read_le16(prob_data + (i + 1) * 4 + 2),
                read_le16(prob_data + (i + 2) * 4 + 2), read_le16(prob_data + (i + 3) * 4 + 2)};
...
void compute_dosages_32bit_simd(const uint8_t* prob_data, float* output, size_t n_samples,
    ...
    if (g_has_avx2) {
        ...
        for (; i + 3 < n_samples; i += 4) {
            ...
            __m256d prob_ab_d =
                _mm256_set_pd(static_cast<double>(prob_ab[3]), static_cast<double>(prob_ab[2]),
                              static_cast<double>(prob_ab[1]), static_cast<double>(prob_ab[0]));

            __m256d prob_bb_d =
                _mm256_set_pd(static_cast<double>(prob_bb[3]), static_cast<double>(prob_bb[2]),
                              static_cast<double>(prob_bb[1]), static_cast<double>(prob_bb[0]));
    ...
    if (g_has_avx2) {
        ...
        for (; i + 7 < n_indices; i += 8) {
            ...
            __m256 missing_mask_vec = _mm256_set_ps(
                is_missing[7] ? std::nanf("") : 0.0f, is_missing[6] ? std::nanf("") : 0.0f,
                is_missing[5] ? std::nanf("") : 0.0f, is_missing[4] ? std::nanf("") : 0.0f,
                is_missing[3] ? std::nanf("") : 0.0f, is_missing[2] ? std::nanf("") : 0.0f,
                is_missing[1] ? std::nanf("") : 0.0f, is_missing[0] ? std::nanf("") : 0.0f);
...
static void compute_dosages_filtered_16bit_simd(const uint8_t* prob_data, float* output,
    ...
    if (g_has_avx2) {
        ...
        for (; i + 7 < n_indices; i += 8) {
            // Gather data for 8 selected samples
            __m256i prob_aa = _mm256_set_epi32(read_le16(prob_data + sample_indices[i + 7] * 4),
            ...
            __m256i prob_ab =
                _mm256_set_epi32(read_le16(prob_data + sample_indices[i + 7] * 4 + 2),
    ...
    if (g_has_neon) {
        ...
        for (; i + 3 < n_indices; i += 4) {
            // Gather data for 4 selected samples
            uint16x4_t prob_aa = {read_le16(prob_data + sample_indices[i + 0] * 4),
                                  read_le16(prob_data + sample_indices[i + 1] * 4),
                                  read_le16(prob_data + sample_indices[i + 2] * 4),
                                  read_le16(prob_data + sample_indices[i + 3] * 4)};

            uint16x4_t prob_ab = {read_le16(prob_data + sample_indices[i + 0] * 4 + 2),
                                  read_le16(prob_data + sample_indices[i + 1] * 4 + 2),
                                  read_le16(prob_data + sample_indices[i + 2] * 4 + 2),
                                  read_le16(prob_data + sample_indices[i + 3] * 4 + 2)};
