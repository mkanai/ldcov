...
public:
    Impl(const std::string& bgi_path)
        : db_(nullptr)
        , variant_count_(0) {
        
        // Open SQLite database (removed NOMUTEX for better internal optimization)
        int rc = sqlite3_open_v2(bgi_path.c_str(), &db_, 
    ...
    ~Impl() {
        ...
        if (stmt_by_region_) sqlite3_finalize(stmt_by_region_);
        if (stmt_by_position_) sqlite3_finalize(stmt_by_position_);
        if (stmt_by_rsid_) sqlite3_finalize(stmt_by_rsid_);
        if (stmt_by_varid_) sqlite3_finalize(stmt_by_varid_);
        if (stmt_by_index_) sqlite3_finalize(stmt_by_index_);
        if (stmt_offsets_range_) sqlite3_finalize(stmt_offsets_range_);
        if (stmt_chromosomes_) sqlite3_finalize(stmt_chromosomes_);
        
        // Close database
        if (db_) sqlite3_close(db_);
    }
    
    std::vector<VariantInfo> query_region(const std::string& chromosome,
    ...
    std::vector<VariantInfo> query_position(const std::string& chromosome,
                                           uint32_t position) {
        read_lock lock(mutex_);
    ...
    VariantInfo get_variant(size_t index) {
        ...
        if (results.empty()) {
            throw std::runtime_error("Failed to get variant at index: " + 
                                   std::to_string(index));
        }
    ...
    std::vector<uint64_t> get_file_offsets(size_t start_idx, size_t end_idx) {
        ...
        if (use_offset_for_range_) {
            ...
            sqlite3_bind_int64(stmt_offsets_range_, 1, static_cast<sqlite3_int64>(end_idx - start_idx));
    ...
    std::vector<std::string> get_chromosomes() const {
        ...
        while ((rc = sqlite3_step(stmt_chromosomes_)) == SQLITE_ROW) {
            const char* chr = reinterpret_cast<const char*>(
                sqlite3_column_text(stmt_chromosomes_, 0));
            if (chr) {
    ...
    std::vector<VariantInfo> find_variants_by_filter(
        ...
        for (size_t i = 0; i < unique_pos_vec.size(); i += batch_size) {
            ...
            while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
                ...
                if (a1) info.allele1 = a1;
                
                const char* a2 = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 7));
                if (a2) info.allele2 = a2;
                else info.allele2 = "";  // Handle NULL as empty string
                ...
                if (it != allele_map.end()) {
                    for (const auto& match : it->second) {
                        if (info.allele1 == match.allele1 && 
                            info.allele2 == match.allele2) {
    ...
    std::vector<VariantInfo> query_positions_batch(
        ...
        all_results.reserve(positions.size() * 2); // Reserve extra space for multiple variants per position
    ...
    std::vector<VariantInfo> query_variant_ids_batch(
        ...
        for (size_t i = 0; i < unique_ids_vec.size(); i += batch_size) {
            ...
            int param_idx = 1;
            for (size_t j = 0; j < batch_size_actual; ++j) {
                sqlite3_bind_text(stmt, param_idx++, unique_ids_vec[i + j].c_str(), -1, SQLITE_STATIC);
            }
...
private:
    ...
    void prepare_statements() {
        ...
        if (sqlite3_prepare_v2(db_, offsets_query, -1, &stmt_offsets_range_, nullptr) != SQLITE_OK) {
            ...
            if (sqlite3_prepare_v2(db_, offset_query, -1, &stmt_offsets_range_, nullptr) != SQLITE_OK) {
...
std::vector<VariantInfo> BGIReader::query_region(
    const std::string& chromosome,
    uint32_t start_pos,
    uint32_t end_pos) {
...
std::vector<VariantInfo> BGIReader::query_position(
    const std::string& chromosome,
    uint32_t position) {
...
std::vector<VariantInfo> BGIReader::query_positions_batch(
    const std::string& chromosome,
    const std::vector<uint32_t>& positions,
    size_t batch_size) {
