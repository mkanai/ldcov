...
std::string BGICache::ensureLocalBGI(const std::string& bgi_path) {
    ...
    // Download from GCS
    std::cout << "Downloading BGI index from " << bgi_path << " to " << local_path << "..."
              << std::endl;
    ...
    try {
        ...
        if (!gcsfs_module) {
            ...
            throw std::runtime_error(
                "Failed to import gcsfs module. Please install: pip install gcsfs");
        }
        ...
        PyObject* args = PyTuple_Pack(2, PyUnicode_FromString(gcs_path.c_str()),
