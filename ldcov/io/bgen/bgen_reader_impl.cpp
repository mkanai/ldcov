...
public:
    explicit MMapFileReader(const std::string& filename) 
        : filename_(filename), data_(nullptr), file_size_(0), 
          current_pos_(0), fd_(-1) {
...
public:
    ...
    Impl(const std::string& filename, const std::string& bgi_filename)
        : filename_(filename), bgi_filename_(bgi_filename), 
          file_reader_(), bgi_reader_(), decompressor_(),
          header_(), sample_ids_(), sample_filter_(),
          is_open_(false), num_threads_(0), decompressor_type_("adaptive") {
    ...
    void set_sample_filter(const std::vector<uint32_t>& indices) {
        ...
        for (auto idx : sample_filter_) {
            if (idx >= header_.nsamples) {
                throw std::runtime_error("Sample index " + std::to_string(idx) + 
                                       " out of range (max: " + 
                                       std::to_string(header_.nsamples - 1) + ")");
            }
        }
    }
    ...
    VariantMetadata read_variant_metadata(uint64_t offset) {
        ...
        auto parse_result = VariantParser::parse(
            buffer.data(), bytes_read, layout_type, compression_type, header_.nsamples
        );
    ...
    std::unique_ptr<decompress::DecompressedData> read_variant_genotypes(const VariantMetadata& metadata) {
        if (!is_open_) {
            throw std::runtime_error("BGEN file is not open");
        }
        ...
        if (header_.layout == 2) { // v1.2
            if (comp_type != decompress::CompressionType::None) {
                ...
                uncompressed_size = data_ptr[0] | (data_ptr[1] << 8) | 
                ...
                size_t max_expected_size = header_.nsamples * 10 + 1000; // Very generous upper bound
                if (uncompressed_size > max_expected_size || uncompressed_size == 0) {
                    throw std::runtime_error(
                        "Invalid uncompressed size in BGEN file: " + 
                        std::to_string(uncompressed_size) + " bytes. "
                        "Expected at most " + std::to_string(max_expected_size) + " bytes for " +
        ...
        decompress::CompressedVariant comp_variant(
        ...
        return std::unique_ptr<decompress::DecompressedData>(new decompress::DecompressedData(std::move(result)));
    }
    ...
    std::vector<std::unique_ptr<decompress::DecompressedData>> read_variants_batch(
        ...
        for (const auto& metadata : variants) {
            // Read compressed genotype data
            auto compressed_data = std::unique_ptr<std::vector<uint8_t>>(new std::vector<uint8_t>(
                metadata.genotype_length));
            ...
            if (header_.layout == 2) { // v1.2
                ...
                if (uncompressed_size > max_expected_size || uncompressed_size == 0) {
                    throw std::runtime_error(
                        "Invalid uncompressed size: " + std::to_string(uncompressed_size) + " bytes. "
                        "Expected at most " + std::to_string(max_expected_size) + " bytes for " +
                        std::to_string(header_.nsamples) + " samples."
                    );
                }
            ...
            compressed_variants.emplace_back(
                metadata.file_offset,
                data_ptr,
                compressed_size,
                uncompressed_size,
        ...
        for (auto& result : results) {
            ptr_results.push_back(std::unique_ptr<decompress::DecompressedData>(new decompress::DecompressedData(std::move(result))));
        }
        return ptr_results;
    }
    ...
    void set_decompressor_type(const std::string& type) {
        ...
        if (type == "sequential") {
            decompressor_ = decompress::create_sequential_decompressor(
                file_reader_.get(), config, true);
        } else if (type == "parallel") {
            decompressor_ = decompress::create_parallel_decompressor(
                file_reader_.get(), num_threads_, config);
        } else if (type == "adaptive") {
            decompressor_ = decompress::create_adaptive_decompressor(
                file_reader_.get(), config);
        } else {
...
private:
    ...
    void parseHeader() {
        ...
        size_t header_size = BgenHeaderParser::getHeaderSize(
            initial_buffer.data(), bytes_read);
    ...
    void openIndex() {
        try {
            ...
            throw std::runtime_error("Failed to open BGI index: " + 
                                   std::string(e.what()));
        }
    }
    ...
    void readSampleIds() {
        if (header_.has_sample_ids) {
            ...
            uint32_t block_size_with_n = size_buffer[0] | 
                                        (size_buffer[1] << 8) | 
                                        (size_buffer[2] << 16) | 
                                        (size_buffer[3] << 24);
            ...
            if (block_size_with_n < 4 || block_size_with_n > 100000000) {  // 100MB max
                throw std::runtime_error(
                    "Invalid sample block size: " + std::to_string(block_size_with_n));
            }
            ...
            // Parse sample block
            auto sample_block = SampleBlockParser::parse(
                sample_buffer.data(), bytes_read, header_.nsamples);
...
BgenReaderImpl::BgenReaderImpl(const std::string& filename, 
                               const std::string& bgi_filename)
