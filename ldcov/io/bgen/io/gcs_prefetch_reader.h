...
class GCSPrefetchReader : public GCSFileReader {
   public:
    ...
    void set_prefetch_size(size_t size) {
        prefetch_size_ = size;
    }
...
// Implementation
inline GCSPrefetchReader::GCSPrefetchReader(const std::string& filename) : GCSFileReader(filename) {
...
inline void GCSPrefetchReader::prefetch_worker() {
    while (!stop_prefetching_) {
        ...
        prefetch_cv_.wait(lock, [this] {
            return stop_prefetching_ || !prefetch_buffers_[1 - active_prefetch_buffer_].ready;
        });

        if (stop_prefetching_)
            break;
        ...
        try {
            ...
            buffer.valid_size = GCSFileReader::read(
                buffer.data.data(), std::min(prefetch_size_, size() - prefetch_offset));
...
inline bool GCSPrefetchReader::try_read_from_prefetch(uint64_t offset, uint8_t* buffer, size_t size,
                                                      size_t& bytes_read) {
    // Check both buffers
    for (int i = 0; i < 2; ++i) {
        const auto& prefetch_buffer = prefetch_buffers_[i];

        if (!prefetch_buffer.ready)
            continue;
        ...
        if (offset >= prefetch_buffer.offset &&
            ...
            // Copy data
            std::memcpy(buffer, prefetch_buffer.data.data() + buffer_offset, bytes_read);
