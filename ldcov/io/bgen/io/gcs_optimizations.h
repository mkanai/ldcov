...
class OptimizedGCSReader : public FileReader {
   ...
   private:
    ...
    std::vector<std::pair<uint64_t, std::vector<uint8_t>>> batch_read(
...
class GCSVariantDecompressor : public VariantDecompressor {
   public:
    ...
    std::vector<uint8_t> decompress_variant(uint64_t offset,
                                            const VariantMetadata& metadata) override;

    void decompress_variants_batch(
        const std::vector<uint64_t>& offsets, const std::vector<VariantMetadata>& metadata,
