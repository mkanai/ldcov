...
public:
     ...
     */
    static Result execute_with_retry(
        ...
        throw std::runtime_error(
            ...
            std::to_string(max_retries + 1) + " attempts. Last error: " + last_error
        );
    }
...
private:
     ...
     */
    static void log_retry(const std::string& operation, size_t attempt, 
                         size_t delay_ms, const std::string& error) {
        // In production, this would use a proper logging framework
        // For now, we'll just use stderr to avoid Python GIL issues
        fprintf(stderr, "[GCS Retry] Operation '%s' attempt %zu failed: %s. "
                       "Retrying in %zu ms...\n", 
...
public:
    static void execute_with_retry(
