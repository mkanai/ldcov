...
@pytest.fixture(scope="session")
def loaded_test_data(bgen_file, bgi_file, sample_file) -> Tuple[np.ndarray, pd.DataFrame, List[str]]:
...
def skip_if_no_ldstore(func):
    ...
    return pytest.mark.skipif(
        not ldstore_available,
        reason="LDstore not available"
    )(func)
...
def skip_if_slow(func):
    ...
    return pytest.mark.skipif(
        os.environ.get("LDCOV_FAST_TESTS", "0") == "1",
